...
| CI / EAS secrets               | `SENTRY_AUTH_TOKEN`, `SENTRY_ORG`, `SENTRY_PROJECT` as build-time-only secrets (EAS secrets + GitHub Actions secrets) — never `EXPO_PUBLIC_*`, since the auth token can create releases/upload source maps and must not ship in the client bundle.                                                                                                                                                                                           |
| `.github/workflows/deploy.yml` | Add the source-map-upload step for the web (Vercel) build, tagged with the git SHA as the release. Native source-map upload is handled by the EAS Build config-plugin hook instead (runs as part of `eas build`, not this workflow).                                                                                                                                                                                                         |
