...
/**
 * First/last name Google handed us with the OAuth identity, for prefilling the
 * name screen. Email OTP signups have no metadata, so both come back ''.
 *
 * Google sets given_name/family_name; `name`/`full_name` is the fallback for
 * identities that only carry the display name, split on the first space so a
 * multi-word surname stays intact.
 */
export function getOAuthName() {
  const m = cachedSession?.user?.user_metadata ?? {};
  ...
  return gap === -1
    ? { first: full, last: '' }
    : { first: full.slice(0, gap), last: full.slice(gap + 1).trim() };
}
...
export async function isOnboardingComplete() {
  const uid = getCurrentUserId();
  if (!uid) return false;
  const { data } = await supabase.from('profiles').select('onboarding_done').eq('id', uid).single();
