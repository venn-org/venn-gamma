...
describe('matchesPrefs', () => {
  ...
  test('a candidate missing the compared field is never excluded', () => {
    // I want a specific budget range, but the candidate hasn't set theirs.
    expect(matchesPrefs({ budgetMin: 20000, budgetMax: 35000 }, {})).toBe(true);
  });

  test('non-overlapping budget ranges exclude the candidate', () => {
    expect(matchesPrefs({ budgetMin: 20000, budgetMax: 35000 }, { budget_min: 0, budget_max: 10000 })).toBe(false);
  });

  test('overlapping budget ranges include the candidate', () => {
    expect(matchesPrefs({ budgetMin: 20000, budgetMax: 35000 }, { budget_min: 25000, budget_max: 40000 })).toBe(true);
  });
  ...
  test('move-in dates within the tolerance window include the candidate, far apart excludes them', () => {
    const prefs = { moveInDate: '2026-08-01' };
    expect(matchesPrefs(prefs, { move_in_date: '2026-08-20' })).toBe(true);
    expect(matchesPrefs(prefs, { move_in_date: '2027-03-01' })).toBe(false);
  });
...
describe('calculateOverlapScore', () => {
  ...
  test('only scores criteria set on both sides — an unset field is not counted', () => {
    // Only budget is comparable (moveInDate is only set on my side); it
    // matches, so the score should be a clean 100, not diluted by the unset field.
    const prefs = { budgetMin: 20000, budgetMax: 35000, moveInDate: '2026-08-01' };
  ...
  test('partial agreement lands strictly between 0 and 100', () => {
    const prefs = { budgetMin: 20000, budgetMax: 35000, moveInDate: '2026-08-01' };
    const candidate = { budget_min: 25000, budget_max: 40000, move_in_date: '2027-03-01' };
