...
/**
 * Vector icon per option, keyed by enum group and DB value. Names are Ionicons
 * unless prefixed `mci:` (MaterialCommunityIcons — Ionicons has no cigarette,
 * drumstick or sprout glyph). Render via components/OptionIcon.
 *
 * The UI labels in ENUMS still carry their emoji because they double as the
 * uiToDb keys and as persisted draft values; `optionText` strips the emoji for
 * display so a screen shows either the icon or the emoji, never both.
 */
export const OPTION_ICONS = {
  pref_role: { seeking: 'search-outline', owner: 'home-outline' },
  pref_gender: { women_only: 'female-outline', men_only: 'male-outline', any_gender: 'male-female-outline' },
  gender: {
    man: 'male-outline', woman: 'female-outline', non_binary: 'male-female-outline',
    transgender: 'color-palette-outline', other: 'color-palette-outline',
    prefer_not_to_say: 'help-circle-outline',
  },
  occupation: {
    working_professional: 'briefcase-outline', student: 'school-outline',
    freelancer: 'laptop-outline', entrepreneur: 'rocket-outline',
  },
  food_habit: {
    veg_only: 'nutrition-outline', eggetarian_ok: 'egg-outline',
    non_veg_ok: 'mci:food-drumstick', vegan_only: 'mci:sprout-outline',
  },
  smoking_pref: { non_smoker: 'mci:smoking-off', smoker_ok: 'mci:smoking', outside_only: 'home-outline' },
  drinking_pref: { teetotaller_only: 'ban-outline', social_drinker_ok: 'wine-outline', fine_with_drinking: 'beer-outline' },
  pets_pref: {
    have_pet: 'paw-outline', fine_with_pets: 'checkmark-circle-outline',
    no_pets: 'close-circle-outline', allergic: 'medkit-outline',
  },
  lifestyle: {
    yes: 'checkmark-outline', sometimes: 'remove-outline',
    no: 'close-circle-outline', prefer_not_to_say: 'help-circle-outline',
  },
};

/** Icon for an option given either its UI label or its DB value. */
export function optionIcon(category, value) {
  const icons = OPTION_ICONS[category];
  if (!icons || !value) return null;
  return icons[value] ?? icons[toDb(category, value)] ?? null;
}

/** Drops a label's leading emoji. Only safe to call once an icon replaces it —
 * it would also eat the '₹' from a budget label. */
export function stripLabelEmoji(label) {
  return typeof label === 'string' ? label.replace(/^[^\p{L}\p{N}]+/u, '') : label;
}

/**
 * Icon + label for rendering one option. The emoji is only stripped when an
 * icon replaces it — groups without icons (budget, age, flat type) keep their
 * label verbatim, so leading '₹' survives.
 */
export function optionDisplay(category, label) {
  const icon = optionIcon(category, label);
  return icon ? { icon, text: stripLabelEmoji(label) } : { icon: null, text: label };
}
...
// Budget/move-in used to be bucketed enums (pref_budget/pref_move_in) edited
// via chips. They're now precise values edited via a slider/calendar, backed
// by budget_min/budget_max/move_in_date — these format them for display.
// pref_budget/pref_move_in are left in the schema but no longer read/written.
export function formatBudgetRange(min, max) {
  if (min == null || max == null) return null;
  if (min === 0 && max === 20000) return null; // the sheet's default range — treat as unset
  const fmt = (n) => (n >= 1000 ? `₹${Math.round(n / 1000)}k` : `₹${n}`);
  return `${fmt(min)} – ${fmt(max)}`;
}

export function formatMoveInDate(dateStr) {
...
export function mapDbPrefsToUI(me) {
  ...
  // pref_role is the user's own role ("I am..."); the feed inverts it to decide
  // whose profiles to show.
  const fallbackRole = me.user_type;
  ...
  return {
    ...
    budget:     formatBudgetRange(me.budget_min, me.budget_max),
    budgetMin:  me.budget_min,
    budgetMax:  me.budget_max,
    moveIn:     formatMoveInDate(me.move_in_date),
