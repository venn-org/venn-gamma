...
export const TERMS_DOC = {
  ...
  blocks: [
    ...
    { type: 'p', text: 'The Service relies on third-party infrastructure, including Supabase for authentication and data storage, and may use Google Sign-In. Your use of those integrations is also subject to the relevant provider\'s own terms.' },
...
export const PRIVACY_DOC = {
  ...
  blocks: [
    ...
    { type: 'ul', items: [
      'Contact & account: email address and, if you use Google Sign-In, your Google account identifiers.',
    ...
    { type: 'ul', items: [
      'Authentication tokens and sign-in metadata from Supabase Auth.',
    ...
    { type: 'ul', items: [
      'To create and operate your account, and to authenticate you securely via Supabase Auth.',
    ...
    { type: 'ul', items: [
      ...
      'Service providers: we use Supabase (PostgreSQL) for authentication and data storage, and Google for optional Google Sign-In; both process data on our behalf under their own data-processing terms and industry-standard security controls, including row-level security policies that restrict who can read or write which records.',
    ...
    { type: 'p', text: 'We use industry-standard safeguards, including encrypted connections (HTTPS), Supabase-issued authentication tokens validated on every request, and database-level row-level-security policies restricting each user to their own data and data they\'re authorized to see (e.g. matched conversations). No method of transmission or storage is 100% secure, and we cannot guarantee absolute security.' },

    { type: 'h', text: '9. International Data Transfers' },
    { type: 'p', text: 'Our service providers (Supabase, and Google where you use Google Sign-In) may process and store data in data centers located outside your country of residence. Where required, we rely on appropriate contractual and technical safeguards for such transfers.' },
...
export const COOKIE_DOC = {
  ...
  blocks: [
    ...
    { type: 'ul', items: [
      'Supabase Auth session storage, so you stay signed in between visits.',
      'Temporary, locally-stored counters, used to rate-limit sign-in code requests and prevent email abuse (never sent to our servers).',
      'Temporary storage of the email address you are signing in with, so we can verify the code you enter.',
    ] },
