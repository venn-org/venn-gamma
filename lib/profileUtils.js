...
export function buildProfileTraits(profile) {
  ...
  // `icon` is an Ionicons name — the same outline set the info card uses, so
  // the chips match the rest of the card instead of mixing in emoji. Ionicons
  // has no cigarette glyph, so tobacco borrows one from MaterialCommunityIcons
  // via `iconSet`.
  const fields = [
    { key: 'drink', icon: 'wine-outline', name: 'Drinks' },
    { key: 'tobacco', icon: 'smoking', iconSet: 'mci', name: 'Smokes' },
    { key: 'weed', icon: 'leaf-outline', name: '420' },
  ];

  const traits = [];
  for (const { key, icon, iconSet, name } of fields) {
    const raw = profile[key];
    if (!raw || raw === 'prefer_not_to_say') continue;
    traits.push({ group: key, icon, iconSet, label: `${name} · ${toUI('lifestyle', raw)}` });
  }
  return traits;
}
